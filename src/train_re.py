...
from utils import DATA_DIR, MODEL_DIR, RESULT_DIR, BERT_MODEL_NAME
...
def get_bert_vectors(sentences):
    print("   -> Đang tính toán lại BERT cho dữ liệu mới ")
...
def get_models():
    return {
        ...
        'RandomForest': RandomForestClassifier(n_estimators=200, class_weight=None, random_state=42, max_depth=15),
        'MLPDeepLearning': MLPClassifier(hidden_layer_sizes=(128, 64), max_iter=1000, early_stopping=True, random_state=42)
    }
...
def main():
    ...
    for vec_name in vector_types:
        ...
        for model_name, model in models_dict.items():
            ...
            print(f"-> F1-Macro={f1:.4f} | Prec={prec:.4f} | Rec={rec:.4f}")
            results.append({
                'Vectorizer': vec_name, 
                'Model': safe_name, 
                'F1_Macro': f1, 
