...
class WeightedTrainer(Trainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        # Đảm bảo trọng số nằm cùng thiết bị (CPU/GPU) với logits
        if self.class_weights.device != logits.device:
            self.class_weights = self.class_weights.to(logits.device)
            
        # Tính CrossEntropyLoss với trọng số đã tính toán
...
def get_all_original_labels(file_paths):
    """Quét toàn bộ dữ liệu để lấy danh sách nhãn gốc (Bệnh, Thuốc...)"""
    original_labels = set()
...
def convert_to_dataset_format(json_data):
    """Chuyển JSON Label Studio sang format Dataset"""
    formatted_data = []
...
def tokenize_and_align_labels(examples, tokenizer, label2id):
    """Tokenize văn bản và gán nhãn BIO tương ứng cho từng token"""
    ...
    for i, text in enumerate(examples["text"]):
        ...
        for token in tokens:
            ...
            # Tìm vị trí token trong văn bản gốc
...
def calculate_class_weights(dataset, label_list, id2label):
    ...
    # Làm mềm trọng số bằng căn bậc 2 (Square Root Smoothing)
    weights_vector = np.power(weights_vector, 0.5)
    
    print("\nBẢNG TRỌNG SỐ LỚP (WEIGHTS):")
...
def main():
    ...
    # Hàm metrics wrapper để truyền id2label vào
